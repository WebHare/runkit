...
if [ -n "$ASSERVICE" ]; then
  while read -r unit; do
    REQUIREUNITS+=("$unit")
  done < <(grep -v '^#' "$WHRUNKIT_TARGETDIR"/required-units 2>/dev/null || true)
...
#    # Allocate a free IP address under
  ...
  if [ -f "$WHRUNKIT_TARGETDIR/docker-options" ]; then
    while IFS= read -r line; do
      DOCKEROPTS+=("$(printf '%q' "$line")")
...
# Tell systemd to use podman (or docker) for shutdowns or it will try to signal conmon which won't understand
