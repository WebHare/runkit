...
exit_syntax()
{
  echo "Syntax: startup-proxy-and-webhare.sh containers..."
  exit 1
}
...
for CONTAINER in "$@" ; do
  # FIXME 'started' should be enough but webhare doesn't announce that state, instead it pretends to be able to annouce 'poststart' but never does
